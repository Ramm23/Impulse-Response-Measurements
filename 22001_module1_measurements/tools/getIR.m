...
% Number of channels
M = size(y,2)
Nfft = length(Hinv); % Length of fft
h = zeros(Nfft, M); % h dimensions [Nfft x M]


% Loop over channels
for i = 1:M
    yi = y(:, i);               % Select the i-th channel
    Y = fft(yi, length(Hinv));  % FFT with zero-padding of length Hinv
    Hest = Y .* Hinv;           % Apply inverse filter
    h(:,i) = ifft(Hest);             % Back to time domain
end
